...
import { Trash2, FileWarning, Pencil } from "lucide-react";
import {
  Table,
  TableBody,
  TableCell,
...
function PermissionTable({ permissions, onEdit, canEdit = false, onDelete, canDelete = false }) {
      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-muted/50 hover:bg-muted/50">
            ...
            <TableHead className="font-bold text-foreground min-w-[130px]">Waktu Keluar</TableHead>
            <TableHead className="font-bold text-foreground min-w-[130px]">Waktu Kembali</TableHead>
                    ...
                    {item.tanggalKembali ? (
                      <>
                        <span className="text-sm font-medium text-foreground">
                          {format(new Date(item.tanggalKembali), "dd MMM yyyy", { locale: idLocale })}
                        </span>
                        <span className="text-xs text-muted-foreground">{item.jamKembali || "-"}</span>
                      </>
                    ) : (
