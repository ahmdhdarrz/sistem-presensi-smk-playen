...
import { importSiswa } from "@/services/siswaService";

const REQUIRED_HEADERS = ["nama", "nis", "kelas_id", "jenis_kelamin"];

function StudentImportDialog({ open, onOpenChange, classes = [], onSuccess, showToast }) {
  ...
  const processExcelFile = async (file) => {
    ...
    try {
      ...
      if (missingHeaders.length > 0) {
        setHeaderError(
          `Header Excel tidak sesuai. Kolom wajib yang hilang: ${missingHeaders.join(", ")}. Format header wajib: nama, nis, kelas_id, jenis_kelamin`
        );
        setParsing(false);
        return;
      }
      ...
      const colMap = {
        ...
        nis: rawHeaders.indexOf("nis"),
        kelas_id: rawHeaders.indexOf("kelas_id"),
        jenis_kelamin: rawHeaders.indexOf("jenis_kelamin"),
      };

      // Valid class IDs list for validation
      ...
      for (let i = headerRowIndex + 1; i < rawRows.length; i++) {
        ...
        const excelRowNum = i + 1; // 1-based index in Excel
        const nama = String(row[colMap.nama] || "").trim();
        const nis = String(row[colMap.nis] || "").trim();
        const kelas_id = String(row[colMap.kelas_id] || "").trim();
        const jenis_kelamin = String(row[colMap.jenis_kelamin] || "").trim().toUpperCase();

        const rowErrors = [];

        if (!nama) rowErrors.push("Nama siswa wajib diisi.");
        if (!nis) rowErrors.push("NIS wajib diisi.");

        if (!kelas_id) {
          rowErrors.push("kelas_id wajib diisi.");
        } else if (isNaN(Number(kelas_id))) {
          rowErrors.push(`kelas_id '${kelas_id}' harus berupa angka ID kelas.`);
        } else if (classes.length > 0 && !validClassIds.has(kelas_id)) {
          rowErrors.push(`kelas_id '${kelas_id}' tidak ditemukan di daftar kelas.`);
        }

        if (!jenis_kelamin) {
          rowErrors.push("jenis_kelamin wajib diisi.");
        } else if (!["L", "P"].includes(jenis_kelamin)) {
          rowErrors.push(`jenis_kelamin '${jenis_kelamin}' tidak valid (hanya L atau P).`);
        }
        ...
        results.push({
          ...
          data: {
            nama,
            nis,
            kelas_id: Number(kelas_id) || kelas_id,
            jenis_kelamin,
          },
  ...
  const handleConfirmImport = async () => {
    ...
    const payload = validRows.map((r) => ({
      nama: r.data.nama,
      nis: r.data.nis,
      kelas_id: Number(r.data.kelas_id),
      jenis_kelamin: r.data.jenis_kelamin,
    }));

    try {
      await importSiswa({ students: payload });
      showToast?.(`${validRows.length} data siswa berhasil diimport.`, "success");
      onSuccess?.();
      ...
      if (status === 404 || status === 405) {
        setBackendNotice(
          "Catatan: API backend Laravel (POST /api/siswa/import) belum diimplementasikan di server. Menunggu ketersediaan endpoint backend."
        );
        showToast?.("Endpoint import di backend Laravel belum tersedia.", "error");
      } else {
        showToast?.(message || "Gagal mengimport data siswa ke server.", "error");
      }
    } finally {
      setIsSubmitting(false);
    }
      ...
      <DialogContent className="sm:max-w-[650px] p-0 overflow-hidden text-left border-border">
        ...
        <DialogHeader className="p-6 pb-4 bg-slate-50/50 dark:bg-slate-900/50 border-b border-border/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-lg font-bold text-foreground">Import Data Siswa Excel</DialogTitle>
              <DialogDescription className="text-xs text-muted-foreground mt-0.5">
                Unggah file Excel (.xlsx / .xls) dengan format header: nama, nis, kelas_id, jenis_kelamin (L/P).
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
          ...
          {parsedRows.length > 0 && !parsing && (
            <div className="space-y-4">
              ...
              {/* Error Detail Log (if errors exist) */}
              {errorRows.length > 0 && (
                    ...
                    <tbody className="divide-y divide-border">
                      {parsedRows.slice(0, 10).map((r) => (
                        <tr key={r.rowNum} className={r.isValid ? "hover:bg-muted/20" : "bg-rose-50/30 dark:bg-rose-950/20"}>
                          ...
                          <td className="py-2 px-3 font-mono text-muted-foreground">{r.data.nis || "-"}</td>
                          <td className="py-2 px-3 text-center font-mono">{r.data.kelas_id || "-"}</td>
                          <td className="py-2 px-3 text-center font-bold">{r.data.jenis_kelamin || "-"}</td>
                          <td className="py-2 px-3 text-center">
                            {r.isValid ? (
                              <Badge className="bg-emerald-100 text-emerald-800 border-emerald-300 dark:bg-emerald-950/60 dark:text-emerald-300 text-[10px] font-bold px-2 py-0">
                                Valid
                              </Badge>
                            ) : (
                              <Badge className="bg-rose-100 text-rose-800 border-rose-300 dark:bg-rose-950/60 dark:text-rose-300 text-[10px] font-bold px-2 py-0">
                                Error
                              </Badge>
                            )}
                          </td>
