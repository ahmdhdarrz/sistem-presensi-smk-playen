    ...
    <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
      ...
      <div className="w-full sm:w-44">
        <Select value={selectedClass} onValueChange={onClassChange}>
          <SelectTrigger id="permission-class-filter" className="bg-card w-full">
      ...
      <p className="text-sm text-muted-foreground whitespace-nowrap shrink-0 font-medium">
        Menampilkan <span className="font-bold text-foreground">{filteredCount}</span> dari{" "}
        <span className="font-bold text-foreground">{totalCount}</span> izin
      </p>
    </div>
  );
}
